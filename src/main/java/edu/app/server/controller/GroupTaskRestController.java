...
public class GroupTaskRestController {
    ...
    public List<GroupTask> findAll() {
        return groupTaskService.getAllGroupTask();
    }

    @PutMapping("/new")
    @ResponseStatus(HttpStatus.CREATED)
    public GroupTask newGroupTask(@Valid @RequestBody GroupTask groupTask) {
        return groupTaskService.saveGroupTask(groupTask);
    }

    @PostMapping("/get/{id}")
    public GroupTask findOne(@PathVariable @Min(1) Long id) {
        return groupTaskService.getById(id);
    }
